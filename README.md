...
2. **Upload both SRT files to Gemini.**
   Attach the English SRT and the target-language SRT to a new Gemini chat.

3. **Start the correction session with the matching guardrail file.**
...
4. **Wait for the handshake, then let Gemini process the files.**
...
5. **Download the corrected SRT from Canvas.**
   Use Canvas's download/export option to save the corrected subtitle file to your machine.

6. **Re-import the corrected SRT into the original video.**
